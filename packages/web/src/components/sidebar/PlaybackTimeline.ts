...
export function renderPlaybackTimeline(options: PlaybackTimelineOptions): string {
  ...
  return `<div class="playback-timeline relative px-3 py-1.5 bg-neutral-950 border-t border-neutral-800 select-none">
    <div class="playback-timeline-track relative h-5 flex items-center cursor-pointer rounded">
    ...
    <div class="playback-timeline-label text-[10px] text-neutral-500 text-center tabular-nums mt-0.5">${currentIndex}</div>
  </div>`;
}
