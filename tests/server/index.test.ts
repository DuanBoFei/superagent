...
describe("WebServer", () => {
  it("starts on the next port when the preferred port is occupied", async () => {
    ...
    const server = new WebServer({ port: occupied.port, maxPortAttempts: 2 });

    try {
      const result = await server.start();
      expect(result.host).toBe("127.0.0.1");
      expect(result.port).toBe(occupied.port + 1);
      expect(result.url).toBe(`http://localhost:${occupied.port + 1}`);
    } finally {
